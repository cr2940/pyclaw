    ...
    def step_hyperbolic(self,solution):
        ...
        if(self.kernel_language == 'Fortran'):
            ...
            if 'method' not in state.problem_data:
                ...
                dtdx_reg = np.zeros( (2*self.num_ghost+grid.num_cells[0]))
                dtdx_reg += self.dt/grid.delta[0]
            ...
            if 'method' not in state.problem_data:
                ...
                wave,s,amdq,apdq, q_hbox, amdq_hbox, apdq_hbox = self.rp(q_l,q_r,aux_l,aux_r,state.problem_data,self.dt,grid.delta[0])
...
# ##################################################################
                cfl = 0.0
                for mw in range(wave.shape[1]):
                    smax1 = np.max(dtdx_reg[LL:UL]*s[mw,LL-1:UL-1]) # for hbox method
                    smax2 = np.max(-dtdx_reg[LL-1:UL-1]*s[mw,LL-1:UL-1])
                    cfl = max(cfl,smax1,smax2)
            ...
            if self.order == 2:
                ...
                if 'method' not in state.problem_data:
                    ...
                    dtdx = dtdx_reg
                    dtdxave = 0.5 * (dtdx_reg[LL-1:UL-1] + dtdx[LL:UL])
                if self.fwave:
                    for mw in range(wave.shape[1]):
